...
function MakeClickSortTable(obj) {
    
    var currentSort = "default"; //needed to reverse the order
    var title = obj.name;
    var sortOrderPropName = obj.sort;
    var sortIcon = obj.icon;
    var objList = obj.list;

    if (!objList){
        var error = document.createElement("div");
        error.innerHTML = "objList parameter was not found and is required";
        return error;
    }
    
    if (!sortOrderPropName){
        var error = document.createElement("div");
        error.innerHTML = "sortOrderPropName parameter was not found and is required";
        return error;
    }
    ...
    function jsSort(objList, byProperty) {
        ...
        objList.sort(function (q, z) {  // in line (and anonymous) def'n of fn to compare list elements. 
            ...
            var c = 0;
            if (currentSort.includes("reverse")) { //if column already sorted, reverse.
                if (qVal > zVal) {
                    c = -1;
                } else if (qVal < zVal) {
                    c = 1;
    ...
    for (var propName in obj) {
        ...
        if (propName[0] !== "_") {
            ...
            headingCell.onclick = function () {
                ...
                if (currentSort === this.sortPropName)
                {
                    currentSort = "reverse" + this.sortPropName;
                }
                else 
                {
                    currentSort = this.sortPropName;
                }
